...
function FormPost(props: propsForm) {
    ...
    const newPost = useRef<typePost>({
        title: '', content: '', userId: '', date: new Date().toString(), reactions: {
            thumbsUp: 0,
            wow: 0,
            heart: 0,
            rocket: 0,
    ...
    return (
        <form>
            ...
            <Select callback={handleAuthor} labelText={'Author'} name={'postAuthor'} changeValue={addPost || false}
                    options={props.users.map(user => ({id: user.id, value: user.name}))}/>
            <Textarea name={'postContent'} callback={handleContent} labelText={'Content post'}
                      changeValue={addPost || false}/>
